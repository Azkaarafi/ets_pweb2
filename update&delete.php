...
<body style="background-color: white">
  ...
  <div class="coicedence">
    <p style="margin-left:-4%;">REMOVE YOUR ORDER</p>
  ...
  <form method="POST" action="">
    <div class="update">
      ...
      <input style="border: 2px solid rgb(182, 24, 24);" class="minitobi" type="submit" value="Submit">
    </div>
  </form>

  <a href="tampilkan.php"> <input style="border: 2px solid rgb(182, 24, 24);" class="minitonok" type=""
      value="Back"></a>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM purchase WHERE name='$name'";
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        if ($password === $row['password']) {
          $found = true;
          $delete_query = "DELETE FROM purchase WHERE id=" . $row['id'];
          if (mysqli_query($conn, $delete_query)) {
            echo "<p class='error1'>Data deletion successful</p>";
          } else {
      ...
      if (!$found) {
        echo '<p class="error1">Wrong password.</p>';
      }
    } else {
      echo '<p class="error1">Name not found.</p>';
    }
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo "<p class='uploader1'>Download File</p>";
    echo "<ul class='uploader2'>";
    while ($row = mysqli_fetch_assoc($result)) {
  ...
  <br />
  <br />
  <hr style="margin-top: 5%; width: 50%; border: 1.4px solid black" />
  <div class="about-me">
    <h2>About Me</h2>
    <p>
